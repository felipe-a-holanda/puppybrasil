...
# for automagic feature addition, Puppy Linux.
#
...
#Modified by LC
  if [ "$AUTO" == "on" ]; then
    ...
    if [ "$status" == "255" ]; then goodbye
      ...
      echo
      #BK changed sh to bash...
      bash $FW_TMPFILE check
      status=$?
