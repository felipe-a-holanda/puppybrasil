...
# echo -e "DEVSTRINGS: $DEVSTRINGS\nCONFIGLIST: $CONFIGLIST\nCONFIGPATTERNS: $CONFIGPATTERNS\nCONFIGMATCH: $CONFIGMATCH\nCONFIGFILE: $CONFIGFILE" >> /tmp/udevtrace-modem.log   #debug
 ...
 #
 #Backup for unknown target IDs
 if [ "`grep 'ok:' /tmp/pup_usb_modeswitch.temp.log`" != "" ];then
  sleep 0.5
  if [ "`grep 'ok:0000:0000' /tmp/pup_usb_modeswitch.temp.log`" != "" ];then
   COUNT=0
   while [ $COUNT -lt 20 ];do
    [ -e $DEVDIR/idProduct ] && break
  ...
  [ "$IDV" != "" -a "$IDP" != "" ] \
   && /usr/sbin/pmodprobeoption.sh $IDV $IDP   
 fi
 return $?
...
#Use tcl-coded wrapper script...
 /usr/bin/tclsh /usr/sbin/usb_modeswitch.sh $ARGUMENTS
 STATUS=$?
 DEVICE="`echo $ARGUMENT1 | cut -f 2 -d /`"
 ...
 return $STATUS
#end tcl-coded wrapper interfacet...
}
